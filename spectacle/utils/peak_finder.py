...
def indexes(y, thres=0.3, min_dist=1, min_thresh=None, max_thresh=None):
    ...
    *thres* and *min_dist* parameters, it is possible to reduce the number of
    ...
    min_thresh = np.min(y) if min_thresh is None else min_thresh
    max_thresh = np.max(y) if max_thresh is None else max_thresh

    thres = thres * (max_thresh - min_thresh) + min_thresh
